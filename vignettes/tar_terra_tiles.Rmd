...
In order to use `tar_terra_tiles()`, we need to break a raster into smaller pieces. We can do that by providing **extent**s used by the raster. The concept of extent is important, so let's unpack that a bit more.

## What is an extent?

The **extent** describes the four points that cover the area of a raster. The extent of a raster, `r`, is printed in the summary:
